...
# Input params:
path_in_positive = '/cs/grad/pazbu/paz/dev/projects/H3K27ac-regression/distal-ranked/join/all.fasta'
path_in_negative = '/cs/grad/pazbu/paz/dev/projects/data/enhancers/NEnhancers.tfa'
path_out = '/cs/grad/pazbu/paz/dev/projects/data/enhancers/ds.114KP.114KN.shuffled.npy'
target_length = 500
...
for header, seq in middle_subseqs(path_in_positive):
    props = header.split('|')
    samples.append(SeqSample(seq, props[0], float(props[1])))
    if c % 1000 == 0:
        print(c)
    c += 1
...
for header, seq in all_subseqs(path_in_negative):
    neg_samples.append(SeqSample(seq, header, 0))
    if c % 1000 == 0:
        print(c)
    c += 1
...
neg_samples = neg_samples[:114000]
